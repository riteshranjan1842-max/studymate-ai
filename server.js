...
app.post('/api/extract-pdf', upload.single('pdf'), async (req, res) => {
  try {
    ...
    // Fix: use default export properly
    const pdf = require('pdf-parse');
    const fn = pdf.default || pdf;
    const data = await fn(req.file.buffer);
    const text = data.text.trim();

    if (!text) return res.status(400).json({ error: 'Could not extract text. Try a text-based PDF.' });
...
app.post('/api/ai', async (req, res) => {
  try {
    ...
    if (history && Array.isArray(history)) {
      messages = [
        { role: 'system', content: 'You are StudyMate AI, a helpful study assistant. Answer any question clearly and educationally.' },
